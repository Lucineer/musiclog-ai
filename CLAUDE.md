# CLAUDE.md — Musiclog Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Musiclog Ai
- **Role**: AI-powered Musiclog companion
- **URL**: https://musiclog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/musiclog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/musiclog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://musiclog-ai.casey-digennaro.workers.dev/health
curl -s https://musiclog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=musiclog-ai
...
- **vessel.json capabilities**: music-logging, playlist-management
...
1. Keep the vessel small. ~320 lines is the current size.
